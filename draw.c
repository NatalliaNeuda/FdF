...
/*   Updated: 2020/02/21 22:57:35 by nneuda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		put_line(t_mlx *mlx, t_line line_cur)
{
	...
	if (mod(x_step) >= mod(y_step))
		while (++i < mod(x_step))
		{
			...
			draw_pixel(mlx, line_cur.x + copysign(i, x_step),
				line_cur.y + copysign(round(delta * y_step), y_step),
				set_color(mlx, line_cur, x_step, i));
		}
	else
		while (++i < mod(y_step))
		{
			delta = (float)i / y_step;
			draw_pixel(mlx, line_cur.x + copysign(round(delta * x_step),
				x_step), line_cur.y + copysign(i, y_step),
				set_color(mlx, line_cur, y_step, i));
		}
}
